      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <FadeIn>
          ...
          {(p.role || p.period) && (
            <dl className="mt-6 flex flex-wrap gap-x-10 gap-y-2 text-sm">
              {p.role && (
                <div>
                  <dt className="text-[11px] uppercase tracking-widest text-secondary font-mono">Role</dt>
                  <dd className="text-foreground font-medium">{p.role}</dd>
                </div>
              )}
              {p.period && (
                <div>
                  <dt className="text-[11px] uppercase tracking-widest text-secondary font-mono">Period</dt>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {p.liveUrl && (
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-on-primary text-sm font-medium hover:opacity-90 transition-opacity"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            ...
            {p.path && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-outline-variant text-sm font-medium text-foreground hover:bg-surface-container transition-colors ${p.path.startsWith("http") ? "" : "cursor-default"}`}
              >
                ...
                {p.path.startsWith("http") ? "View repository" : p.path}
              </a>
            )}
          </div>
        </FadeIn>
        ...
        {p.longDescription && (
          <FadeIn className="mt-14">
            <h2 className="text-[11px] uppercase tracking-widest text-secondary font-mono mb-3">Overview</h2>
        ...
        {p.highlights.length > 0 && (
          <FadeIn className="mt-14">
            <h2 className="text-[11px] uppercase tracking-widest text-secondary font-mono mb-5">Features & Functions</h2>
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {p.highlights.map((h, i) => (
                <li key={i} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-4">
                  <p className="font-medium text-foreground text-sm">{h.title || `Feature ${i + 1}`}</p>
        ...
        {p.images.length > 0 && (
          <FadeIn className="mt-14">
            <h2 className="text-[11px] uppercase tracking-widest text-secondary font-mono mb-5">Screenshots</h2>
            ...
            <div className="flex justify-between gap-6 text-sm">
              {prev ? (
                <Link href={`/projects/${prev.slug}`} className="group flex flex-col">
                  <span className="text-[11px] uppercase tracking-widest text-secondary font-mono">← Previous</span>
              ...
              {next && (
                <Link href={`/projects/${next.slug}`} className="group flex flex-col items-end text-right">
                  <span className="text-[11px] uppercase tracking-widest text-secondary font-mono">Next →</span>
