...
FORCE_SHOW_ALL_JS = r"""
() => {
  // Flip any element with opacity 0 or framer-controlled transform back to visible
  ...
  for (const el of all) {
    ...
    // Framer often sets inline transform via style attribute; reset only if needed
    if (transform && transform !== 'none' && el.hasAttribute('style')) {
      // Don't reset transforms that are part of normal layout (none); only nudge hidden
      const t = transform.match(/matrix\(([^)]+)\)/);
      if (t) {
        const parts = t[1].split(',').map(s => parseFloat(s.trim()));
        if (parts.length === 6) {
          // parts[3], [4] = scaleX, skewY; parts[5], [6] = skewX, scaleY -> but matrix is 6 values
          // Matrix(a,b,c,d,e,f): a=scaleX, b=skewY, c=skewX, d=scaleY, e=translateX, f=translateY
...
def capture_mobile(browser):
    ...
    full_path = OUT_DIR / "nextjs-v2-mobile-375.png"
    page.screenshot(path=str(full_path), full_page=True)
...
def main():
    ...
    # Verify
    for fname in ("nextjs-v2-desktop-fullpage.png", "nextjs-v2-desktop-hero.png", "nextjs-v2-mobile-375.png"):
        p = OUT_DIR / fname
        if p.exists():
            size = p.stat().st_size
